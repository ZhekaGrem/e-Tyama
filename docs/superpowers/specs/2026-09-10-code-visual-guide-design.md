...
| **2 (дефолт)** | так | ≤12 файлів, публічні + приватні функції | **до 3 фрагментів** (один на файл): entry, max_sugar, max_trap |
...
| `operation` | функції, методи, оператори, if/else, цикли | зелений `#10B981` | функція `[прямокутник]`, умова і цикл `{ромб}` | `<rect>` / ромб `<polygon>` |
| `syntax_sugar` | async/await, декоратори, spread, `?.`, `!!`, деструктуризація, comprehension, лямбди | помаранчевий `#F59E0B` | async/декоратор/`?.` `>прапорець]`, spread/деструктуризація/comprehension/лямбда/CTE `{{шестикутник}}` | трикутник `<polygon points="0,0 10,0 5,8">`, шестикутник `<polygon points="5,0 10,2.5 10,7.5 5,10 0,7.5 0,2.5">` |
...
→ Write у `artifacts/`; у JSON кожен `<` записується як `<` (не лише `</`), бо
`<script>`-дані реагують і на `<!--`. Помилка даних → червоний блок «Помилка гайда: …» замість порожньої
...
├─ <constraint> учень не назвав файли, 1 мова → до 3: #1 entry, #2 max_sugar, #3 max_trap; один фрагмент на файл.
│    depth 2 → до 3 незалежно від кількості мов; depth 3 → до 5, щонайменше по 1 на мову </constraint>
