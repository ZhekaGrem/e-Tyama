...
Порядок: ① тіло `def home` створює об'єкт-функцію → ② `app.route("/")` викликається
(декоратор-фабрика) → ③ результат викликається з `home` як єдиним аргументом → ④ ім'я `home`
у просторі імен перепризначається на результат.
Чому: декоратор — це вираз, який обчислюється в момент виконання `def`, тобто при завантаженні
модуля, а не при виклику `home()` пізніше.
