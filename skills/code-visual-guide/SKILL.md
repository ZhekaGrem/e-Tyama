...
| L3 Вираз | літерали, оператори, порядок виконання | навчання «як читає мова»: **до 3 фрагментів** (один на файл) — entry, max_sugar, max_trap |
...
```
Маркери в репо (sql і mongo — будь-де, mongo — за вмістом файлів):
...
<constraint> учень не назвав файли, одна мова → до 3: #1 entry, #2 max_sugar, #3 max_trap; один фрагмент на файл (l3_fragment у файлі один) </constraint>
<constraint> depth 2 → до 3 фрагментів незалежно від кількості мов (entry з головної мови, max_sugar і max_trap по всьому репо); depth 3 → до 5, і щонайменше по одному на мову </constraint>
...
<constraint> фрагментів ніколи не більше, ніж файлів, що підходять (≥ 5 рядків коду або entry point); третій слот не заповнюється вигаданою пасткою: why_chosen "trap" лише з посиланням на конкретний §2.N у lang_<x>.md, інакше — most_sugar-кандидат №2 або слот лишається порожнім </constraint>
...
Виклик функції без крапки (`calculateTotal(items)`) — це operation kind `function` з
explanation «виклик …»; kind `method` — лише для виклику через крапку. Окремого kind для
виклику немає, і це навмисно.
...
2. .md    → <сховище>/code/<назва-репо>/code_guide_<sha8>.md          (джерело; у json поле source_md = цей шлях)
...
<constraint> у JSON перед вставкою заміни кожен `<` на `\u003c` — script-дані реагують не лише на `</script`, а й на `<!--`; JSON.parse відновить символ </constraint>
...
```json
{
  ...
  "root_path": "string",
  "source_md": "string — шлях до цього .md, показується у футері HTML",
...
```
1. sha8:   ключ = вивід `git remote get-url origin` без переносу рядка (немає remote → абсолютний шлях репо); той самий репо через ssh і https дасть різні ключі — це відомо і прийнятно
...
3. JSON  з блоку ```json у code_guide_<sha8>.md; замінити кожен < на \u003c
...
- L2 ≤ 12 файлів (стеля 16); L3 ≤ 12 елементів на фрагмент; фрагментів ≤ 3 (depth 2) / ≤ 5 (depth 3), один на файл
...
> «Поясни своїми словами фрагмент №2 — або №1, якщо фрагмент один — (<файл>, рядки N–M):
> у якому порядку його виконує мова і чому?»
...
3. L3: ≤ 3 фрагменти (depth 2) або ≤ 5 (depth 3), один на файл, жоден слот не заповнений вигаданою пасткою, кожен ≤ 12 елементів?
